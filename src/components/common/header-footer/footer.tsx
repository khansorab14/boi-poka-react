...
import Library1 from "../../../../public/assets/icons/boipoka/Vector.svg";
...
const Footer: React.FC = () => {
  const { showLibrary, setShowLibrary } = useUIStore();
  const { currentTab, setTab } = useNavigateTabStore();
  const { setActiveTab } = useInnerCircleTabStore();
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full border-t bg-white px-4 py-3 grid grid-cols-5 items-end text-xs z-10">
      ...
      <button
        ...
        className={`flex flex-col items-center ${currentTab === "Library" ? "text-black" : "text-gray-400"}`}
      >
      ...
      <button
        ...
        className={`flex flex-col items-center ${currentTab === "Inner Circle" ? "text-black" : "text-gray-400"}`}
      >
      ...
      {/* Tab: My Persona */}
      <button
        onClick={() => setTab("My Persona")}
        className={`flex flex-col items-center ${currentTab === "My Persona" ? "text-black" : "text-gray-400"}`}
      >
      ...
      {/* Tab: I am Bored */}
      <button
        onClick={() => setTab("I am Bored")}
        className={`flex flex-col items-center ${currentTab === "I am Bored" ? "text-black" : "text-gray-400"}`}
      >
